...
| Inventory verification | `npm run inventory:verify` | Revalidate all current queue items against disk |
| Publication brief | `npm run publish:brief -- --position <n>` | Revalidate the exact queue item, copy, settings, media, and hash |
| Browser preparation | `npm run publish:start -- --position <n> ...` | Use the dedicated profile and stop with each requested platform at `ready` |
| Publication status | `npm run publish:status` | Read the current local browser-publication session without changing it |
| Final publication | `npm run publish:confirm -- --platform <name\|all> --confirm-sha <sha256>` | Publish only a ready form with an exact render-hash confirmation |
| Publication verification | `npm run publish:verify -- --platform <douyin\|xiaohongshu> [--session <id>]` | Search the official work list after an ambiguous submission, save a proof screenshot, and record success only on an exact-title match |
...
- Publication, post-publication verification, screenshots, recording, and retries must use the repository's `publish:*` scripts with the ignored dedicated profile. Never substitute direct browser control, Computer Use, an ad hoc Playwright command, a platform API, or another manual/automated fallback.
- An explicit user instruction in the current task to publish or start publishing is the publication confirmation for that run. Do not ask for a second confirmation; still pass the exact prepared SHA-256 to `publish:confirm` as the script's internal integrity gate. Never publish content that has no explicit current-task publication instruction.
- Browser publication must use official creator pages and the ignored dedicated profile. Stop at `ready`, revalidate the account, copy, associated official topics, settings, media path, and exact hash, then let `publish:confirm` click the final control once.
- After the platform accepts a submission, the script must open the official work or article list, verify an exact-title match in a submitted state such as published or under review, and save a list-page screenshot as publication proof. Deliver that screenshot to the user. Update only the verified platform status under the queue lock, and only after both the exact-title list match and proof screenshot succeed.
...
- Count one publication attempt as one script-created platform session for the same platform and release ID. On a definite pre-submit or platform-rejected failure, create a fresh script session and retry the same immutable release, account, copy, topics, and settings. Stop immediately on success. Stop after three definite failures, keep the platform pending, and report the three session IDs and concrete failure reasons.
- An ambiguous submit outcome is not a definite failure and must never trigger an immediate re-upload. Run `publish:verify` through the script first. If the official list contains the exact title, capture proof and record success without republishing; if authoritative verification proves the title absent, the next fresh script session counts as the next attempt. A `published_unrecorded` state must be repaired with script verification or recording, never by republishing.
- Every hashtag in `publish.json` must be selected from each platform's official topic suggestions during browser preparation. Prefer exact matches; a similar official topic is allowed only after explicit user approval and must be recorded as an expected-to-actual mapping in the final review. Plain hashtag text is not sufficient, and any unassociated topic blocks publication before the final button.
- Xiaohongshu's original-declaration control defaults to off. When the workflow has not enabled it and the custom control exposes no readable state, record it as off with `source: user-confirmed-platform-default` and do not block publication; any visible or machine-readable enabled state still blocks.
